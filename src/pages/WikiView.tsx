...
function renderMarkdownToBasicHTML(markdown: string, pageName: string): string {
  ...
  html = html.replace(/!\[\[([^|\]\n]+)(?:\|([^\]\n]+))?\]\]/g, (match, filename, caption) => {
    ...
    // YouTube
    const isYoutube = /^(https?:\/\/)?(www\.|m\.)?(youtube\.com|youtu\.be)\//.test(cleanName);
    if (isYoutube) {
       const regExp = /^.*(youtu\.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/;
       const match = cleanName.match(regExp);
       ...
       return `<div class="my-6 border border-slate-200 rounded-2xl overflow-hidden shadow-sm bg-slate-50 font-sans">
        <div class="px-4 py-2.5 bg-slate-100 border-b border-slate-200 flex items-center justify-between text-xs text-slate-500 font-semibold">
          <span class="flex items-center gap-1.5 truncate">
            <span class="px-1.5 py-0.5 bg-rose-100 text-rose-800 rounded text-[10px] font-bold">YOUTUBE</span> ${cleanName}
          </span>
          <a href="${cleanName}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline font-medium">Open in YouTube</a>
        </div>
        <div class="p-4 bg-white flex flex-col items-center justify-center">
          <iframe src="https://www.youtube.com/embed/${videoId}" title="${cleanName}" class="w-full rounded-lg border border-slate-200 bg-black/5 aspect-video" style="max-height: 480px;" allowfullscreen></iframe>
